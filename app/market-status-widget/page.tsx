...
export default function MarketStatusWidgetPage() {
  const { theme, toggleTheme } = useTheme();
  const [activeDir, setActiveDir] = useState(marketStatusDirections[0].id);
  ...
  const cycleSentiment = () =>
    setSentiment((s) => {
      const idx = sentimentOrder.indexOf(s);
      return sentimentOrder[(idx + 1) % sentimentOrder.length];
    });

  // Detect browser tz after mount to avoid hydration mismatch
  useEffect(() => {
  ...
  const current =
    marketStatusDirections.find((d) => d.id === activeDir) ??
    marketStatusDirections[0];
        ...
        <div className="sticky top-0 z-20 bg-background/90 backdrop-blur-md border-b border-border/40">
          <div className="flex items-center gap-2 px-5 pt-6 pb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[12px] text-muted-foreground truncate">
                {marketStatusDirections.length} directions ·{" "}
                {marketStates.length} states · times in{" "}
          ...
          <div className="overflow-x-auto no-scrollbar">
            <div className="flex items-center gap-1 px-4 pb-3">
              {marketStatusDirections.map((dir) => {
          ...
          {marketStates.map((state, stateIdx) => (
            <div key={state.id}>
              ...
              {activeDir === "typographic" ? (
                ...
                <Component state={state} theme={theme} userTz={userTz} />
              )}
            </div>
          ))}
        </div>
