...
export function MarketStatusCombinedBar({
  ...
  // Gauge geometry (reused from MarketSentimentWidget)
  const cx = 32;
  const cy = 30;
  const r = 24;
  const gapDeg = 6;
  ...
  const segments = sentimentOrder.map((s, i) => {
    ...
    return {
      id: s,
      d: `M ${(cx + r * Math.cos(sRad)).toFixed(2)} ${(cy - r * Math.sin(sRad)).toFixed(2)} A ${r} ${r} 0 0 1 ${(cx + r * Math.cos(eRad)).toFixed(2)} ${(cy - r * Math.sin(eRad)).toFixed(2)}`,
      color: sentimentColors[s],
    };
  });
  ...
  return (
    <div className="-mx-5">
      <div className="flex items-stretch bg-muted">
        {/* Left — Market Status (fit to content, tappable) */}
        <button
        onClick={onToggleStatus}
        className="shrink-0 px-5 py-5 text-left active:opacity-80 transition-opacity"
      >
        <p className="text-[12px] font-extrabold tracking-[0.12em] text-muted-foreground/50 uppercase flex items-center gap-2">
          {word}
          {state.isLive && (
      ...
      {/* Right — Sentiment (fills remaining) */}
      <button
        onClick={onToggleSentiment}
        className="flex-1 min-w-0 px-5 py-5 flex items-center justify-between gap-3 text-left active:opacity-80 transition-opacity"
      >
        <div className="min-w-0">
          <p className="text-[12px] font-extrabold tracking-[0.12em] text-muted-foreground/50 uppercase">
            Market Mood
          </p>
          <p className="mt-1 text-[14px] font-normal text-foreground">
            {sentimentLabels[sentiment]}
          </p>
        </div>
      ...
      {state.notice && (
        <p className="px-5 py-2 text-[12px] font-medium text-amber-800/60 dark:text-amber-300/50 leading-snug bg-amber-50 dark:bg-amber-950/30">
          {state.notice}
        </p>
      )}
    </div>
